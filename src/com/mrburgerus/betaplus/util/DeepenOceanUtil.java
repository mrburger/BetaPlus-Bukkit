...
{
	public static void deepenOcean(BaseChunkGenerator.GeneratingChunk chunk, Random random, int seaLevel, int smoothSize, double scaleFactor)
	{
		// Get X and Z start
		int xStart = chunk.getChunkX() << 4;
		int zStart = chunk.getChunkZ() << 4;
		...
		for (int xV = 0; xV < newDepths.length; ++xV)
		{
			for (int zV = 0; zV < newDepths[xV].length; ++zV)
			{
				...
				if (yNew < y && y < seaLevel) // We are Deep, yo.
				{
					...
					{
						chunk.getBlocksForChunk().setBlock(xStart + xV, yV, zStart + zV, org.bukkit.Material.WATER);
					}
				}
			}
		}
