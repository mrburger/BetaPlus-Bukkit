...
public abstract class AbstractWorldSimulator
{
	...
	/* DATA CACHED HERE */
	/* The Pair is simply a Y Average for the ChunkCoordIntPair and whether any values fall above sea level */
	// 1x1 Averages Cache
	...
	protected HashMap<ChunkCoordIntPair, Pair<boolean[][], Boolean>> beachBlockCache;
	// Holds cache of EVERY simulated y value. Testing for memory issues.
	// REMEMBER TO FILL THIS VALUE.
	protected HashMap<ChunkCoordIntPair, int[][]> chunkYCache;
	...
	/* Simulate, then Average a 3x3 chunk area centered on the ChunkCoordIntPair */
	public Pair<Integer, Boolean> simulateYAvg(BlockPosition blockPos)
	{
		ChunkCoordIntPair ChunkCoordIntPairForUse = new ChunkCoordIntPair(blockPos);
		if (avgYCache.containsKey(ChunkCoordIntPairForUse))
		{
			return avgYCache.get(ChunkCoordIntPairForUse);
		}
		else
		{
			int sum = 0;
			...
			avgYCache.put(ChunkCoordIntPairForUse, retPair);
			return retPair;
		}
	}
	...
	{
		ChunkCoordIntPair ChunkCoordIntPairForUse = new ChunkCoordIntPair(pos);

		if (yCache.containsKey(ChunkCoordIntPairForUse))
		{
			return yCache.get(ChunkCoordIntPairForUse);
		}
		else
		{
			Pair<Integer, Boolean> retPair = getSimulatedAvg(ChunkCoordIntPairForUse);
			yCache.put(ChunkCoordIntPairForUse, retPair);
			return retPair;
		}
	}
	...
	public boolean isBlockBeach(BlockPosition pos)
	{
		ChunkCoordIntPair ChunkCoordIntPair = new ChunkCoordIntPair(pos);
		int xPosChunk = pos.getX() & 15;
		int zPosChunk = pos.getZ() & 15;
		if (beachBlockCache.containsKey(ChunkCoordIntPair))
		{
			return beachBlockCache.get(ChunkCoordIntPair).getFirst()[xPosChunk][zPosChunk]; // Get boolean array at the position in question, if yes it is sand.
		}
		else
		{
			Pair<boolean[][], Boolean> sandPair = isBeachBlockSim(ChunkCoordIntPair);
			beachBlockCache.put(ChunkCoordIntPair, sandPair); // Enter the value
			return sandPair.getFirst()[xPosChunk][zPosChunk];
		}
	}

	private void enterIntoCache(ChunkCoordIntPair cPos, int[][] yValues)
	{
		chunkYCache.put(cPos, yValues);
	}

	// This function is the same for both Alpha and Beta, the noise octaves are different though.
	public Pair<int[][], Boolean> simulateChunkYFull(ChunkCoordIntPair pos)
	{
	...
	private Pair<boolean[][], Boolean> isBeachBlockSim(ChunkCoordIntPair ChunkCoordIntPair)
	{
		...
		this.sandNoise = this.beachNoise.generateNoiseOctaves(this.sandNoise, ChunkCoordIntPair.x * 16, ChunkCoordIntPair.z * 16, 0.0, 16, 16, 1, thirtySecond, thirtySecond, 1.0);
		this.gravelNoise = this.beachNoise.generateNoiseOctaves(this.gravelNoise, ChunkCoordIntPair.x * 16, 109.0134, ChunkCoordIntPair.z * 16, 16, 1, 16, thirtySecond, 1.0, thirtySecond);
		this.stoneNoise = this.surfaceNoise.generateNoiseOctaves(this.stoneNoise, ChunkCoordIntPair.x * 16, ChunkCoordIntPair.z * 16, 0.0, 16, 16, 1, thirtySecond * 2.0, thirtySecond * 2.0, thirtySecond * 2.0);
		for (int z = 0; z < 16; ++z)
		{
			for (int x = 0; x < 16; ++x)
			{
		...
		beachBlockCache.put(ChunkCoordIntPair, retPair);
		return retPair;
	}
	...
	public abstract Pair<int[][], Boolean> simulateChunkYFast(ChunkCoordIntPair pos);
}
