...
import com.mrburgerus.betaplus.world.beta_new.biome.BetaPlusBiomeSelector;
import com.mrburgerus.betaplus.world.beta_new.biome.BetaPlusClimate;
import com.mrburgerus.betaplus.world.beta_new.biome.EnumBetaPlusBiome;
import com.mrburgerus.betaplus.world.beta_new.sim.BetaPlusSimulator;
...
public class BiomeProviderBetaPlus
{
	...
	public double[] noise;
	private static final Biome[] Biome_LIST = buildBiomeList();
	...
	public BiomeProviderBetaPlus(World world)
	{
		...
		*/
		...
		simulator = new BetaPlusSimulator(world.getSeed());
		climateSim = new BetaPlusClimate(world.getSeed(), scaleVal, mult);
	}

	/* Builds Possible Biome List */
	private static Biome[] buildBiomeList()
	{
		EnumBetaPlusBiome[] betaPlusBiome = EnumBetaPlusBiome.defaultB.getDeclaringClass().getEnumConstants();
		Set<Biome> Biomeet = Sets.newHashSet();
		for (int i = 0; i < betaPlusBiome.length; i++)
		{
			Biomeet.add(betaPlusBiome[i].handle);
		}
		return Biomeet.toArray(new Biome[Biomeet.size()]);
	}
	...
	public Set<Biome> a(int centerX, int centerZ, int sideLength)
	{
		...
		Collections.addAll(set, this.generateBiomeWithOceans(centerX, centerZ, sideLength, sideLength, true));
		return set;
	}

	public BlockPosition a(int x, int z, int range, List<Biome> biomeList, Random random)
	{
		int i = x - range >> 2;
		int j = z - range >> 2;
		int k = x + range >> 2;
		...
		int xSize = k - i + 1;
		int zSize = l - j + 1;
		Biome[] biomeArr = this.generateBiome(i, j, xSize, zSize);
	...
	public Biome getOceanBiome(BlockPosition pos, boolean isDeep)
	{
	...
	public Biome getBeachBiome(BlockPosition pos)
	{
		double[] climate = climateSim.getClimateValuesatPos(pos);
