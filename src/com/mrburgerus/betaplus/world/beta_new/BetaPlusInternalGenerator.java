...
import com.mrburgerus.betaplus.world.beta_new.biome.EnumBetaPlusBiome;
...
public class BetaPlusInternalGenerator extends CustomChunkGenerator
{
	...
	public BetaPlusInternalGenerator(World world, ChunkGenerator generator)
	{
		...
		//TESTING
		biomeProviderS = new BiomeProviderBetaPlus(world);
	}
	...
	{
		BetaPlus.LOGGER.log(Level.INFO, "CREATING TWICE");
		// Get Position
		...
		biomesForGeneration = BiomeReplaceUtil.convertBiomeArray(biomeProviderS.getBiomes(x * 16, z * 16, 16, 16));
	...
	private void replaceBiomes(IChunkAccess iChunk)
	{
		for (int z = 0; z < CHUNK_SIZE; ++z)
		{
			for (int x = 0; x < CHUNK_SIZE; ++x)
			{
				...
				{
					biomesForGeneration[(x << 4 | z)] = BiomeReplaceUtil.convertBiome(EnumBetaPlusBiome.mountain.handle);
				}
				else if (yVal < BetaPlus.seaLevel - 1)
				{
					...
					{
						biomesForGeneration[(x << 4 | z)] = BiomeReplaceUtil.convertBiome(biomeProviderS.getOceanBiome(new BlockPosition(xPos, yVal, zPos), true));
					}
					else
					{
						biomesForGeneration[(x << 4 | z)] = BiomeReplaceUtil.convertBiome(biomeProviderS.getOceanBiome(new BlockPosition(xPos, yVal, zPos), false));
					}
				}
			}
		}
	...
	private void replaceBeaches(IChunkAccess chunk)
	{
		for (int z = 0; z < CHUNK_SIZE; ++z)
		{
			...
			for (int x = 0; x < CHUNK_SIZE; ++x)
			{
				...
				if ((yVal <= (BetaPlus.seaLevel + 1) && yVal >= BetaPlus.seaLevel - 1) && (biome != BiomeReplaceUtil.convertBiome(EnumBetaPlusBiome.desert.handle)) && chunk.getType(new BlockPosition(xPos, yVal, zPos)) == Blocks.SAND.getBlockData())
				{
					this.biomesForGeneration[(x << 4 | z)] = BiomeReplaceUtil.convertBiome(biomeProviderS.getBeachBiome(new BlockPosition(xPos, yVal, zPos)));
				}
			}
		}
	}
	...
	private void replaceBlocksForBiome(int chunkX, int chunkZ, IChunkAccess chunkprimer, EnumBetaPlusBiome[] biomes)
	{
		...
		for (int z = 0; z < 16; ++z)
		{
			for (int x = 0; x < 16; ++x)
			{
				...
				int checkVal = -1;
				BlockData topBlock = biome.topBlock.createBlockData();
				BlockData fillerBlock = biome.fillerBlock.createBlockData();
				...
				for (int y = 127; y >= 0; --y)
				{
					...
					else
					{
						...
						if (checkVal == -1)
						{
							...
							{
								topBlock = biome.topBlock.createBlockData();
								fillerBlock = biome.fillerBlock.createBlockData();
								if (gravelN)
								{
									topBlock = Bukkit.createBlockData(org.bukkit.Material.AIR);
