...
public class BiomeProviderBeta
{
	...
	public BiomeProviderBeta(long seedLong)
	{
	...
	private Biome[] generateBiomeWithOceans(int startX, int startZ, int xSize, int zSize, final boolean useAverage)
	{
		...
		for (int x = 0; x < xSize; ++x)
		{
			...
			for (int z = 0; z < zSize; ++z)
			{
				...
				{
					Pair<Integer, Boolean> avg = simulator.simulateYAvg(new BlockPosition(x + startX, 0,  z + startZ));
					// Tried 56, 58, 57
					if (avg.getFirst() < 64 - 5) // Usually 58
					{
				...
				{
					Pair<Integer, Boolean> avg = simulator.simulateYChunk(new BlockPosition(x + startX, 0,  z + startZ));
					if (avg.getFirst() < 64 - 1) // 62 usually
					{
						biomeArr[counter] = this.getOceanBiome(x + startX, z + startZ, false);
					}
					else if (avg.getFirst() >= 64 - 2 && avg.getFirst() <= 64 + 1)
					{
						/* Now, add Beaches */
						if (simulator.isBlockSandSim(new BlockPosition(x + startX, 0,  z + startZ)))
						{
	...
	{
		double[] climate = climateSim.getClimateValuesatPos(new BlockPosition(xPos, 0, zPos));
