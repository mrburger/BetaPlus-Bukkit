...
public class BetaPlusGenerator extends ChunkGenerator
{
	...
	public BiomeProviderBeta biomeProviderS;

	private ArrayList<BlockPopulator> blockPopulator = new ArrayList<BlockPopulator>();

	private boolean hasEnabled;
	public static BetaPlusGenerator INSTANCE;

	public BetaPlusGenerator()
	{
		hasEnabled = false;
		INSTANCE = this;
		blockPopulator.add(new BetaBlockPopulator());
	}
	...
	public void init(long seed)
	{
		...
		biomeProviderS = new BiomeProviderBeta(seed);
	}
	...
	public ChunkData generateChunkData(World world, Random random, int x, int z, BiomeGrid biomeGrid)
	{
		...
		DeepenOceanUtil.deepenOcean(x, z, terrain, rand, BetaPlus.seaLevel, 7);
		...
		this.replaceBiome(x, z, terrain);
		this.replaceBeaches(x, z, terrain);

		// Replaces biomes (TESTING)
		int n = 0;
		Biome[] biomesList = BiomeReplaceUtil.toBukkitBiome(BiomeReplaceUtil.convertBiomeArray(biomesGenerate));
		...
		//BetaPlus.LOGGER.log(Level.FINE, "Finished: " + x + ", " + z);
		return terrain;
	}
	...
	private void replaceBiome(int xC, int zC, ChunkData chunkData)
	{
		for (int z = 0; z < 16; ++z)
		{
			for (int x = 0; x < 16; ++x)
			{
				int xPos = xC << 4 + x;
				int zPos = zC << 4 + z;
				int yVal = BiomeReplaceUtil.getSolidHeightY(new BlockPosition(x, 0, z), chunkData); //world.getHighestBlockYAt(xPos, zPos);
				if (yVal > 108)
				{
	...
	private void replaceBeaches(int xC, int zC, ChunkData data)
	{
		for (int z = 0; z < 16; ++z)
		{
			...
			for (int x = 0; x < 16; ++x)
			{
				int xPos = xC + x;
				int zPos = zC + z;
				int yVal = BiomeReplaceUtil.getSolidHeightY(new BlockPosition(x, 0, z), data);
				// New Line
				Biome biome = biomesGenerate[(x << 4 | z)];
				//Inject Beaches (MODIFIED)
				if ((yVal <= (BetaPlus.seaLevel + 1) && yVal >= BetaPlus.seaLevel - 1) && (biome != EnumBetaPlusBiome.desert.handle) && data.getType(x, yVal, z) == Material.SAND)
				{
					this.biomesGenerate[(x << 4 | z)] = biomeProviderS.getBeachBiome(xPos, zPos);
				}
			}
		}
	}
